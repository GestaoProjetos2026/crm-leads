...
- SaaS multi-tenant
- Backend em Node.js (NestJS preferencialmente)
- PostgreSQL com Row Level Security (RLS)
- Redis para cache
- Sistema baseado em módulos: CRM, Analytics, Automação
...
1. Implemente a estrutura inicial do projeto backend usando NestJS
2. Organize os módulos principais:
   - auth
   - tenants
   - leads
   - opportunities
...
5. Gere:
   - código base
   - arquivos principais
   - explicação curta das decisões (docs\decisions)
6. Documente:
   - status atual do projeto (docs\context\current-state.md)
   - modulos (docs\modules)

## Regras
IMPORTANTE:
- Não implemente lógica de negócio ainda
- Foque apenas na fundação escalável
...
- Documentação atualizada (caso necessário)
- Testes unitários atualizados (caso necessário)

No próximo prompt vamos implementar autenticação multi-tenant.